...
export function bindEditorEvents(initialRecipe, onSave, onNavigate) {
  ...
  // ── Phase toggle ──────────────────────────────────────────────────────────
  document.querySelectorAll(".phase-toggle__btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const si    = btn.dataset.stage;
      const phase = btn.dataset.phase;
      // Update hidden input
      const hidden = document.querySelector(`[name="stage[${si}][phase]"]`);
      if (hidden) hidden.value = phase;
      // Update button states
      btn.closest(".phase-toggle").querySelectorAll(".phase-toggle__btn").forEach((b) => {
        b.classList.toggle("phase-toggle__btn--active", b.dataset.phase === phase);
      });
    });
  });
